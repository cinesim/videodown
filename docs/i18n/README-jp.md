...
[リリースページ](https://github.com/cinesim/videodown/releases/latest)から最新の `videodown-%version%.dmg` をダウンロードするか、[**homebrew cask**](https://github.com/caskroom/homebrew-cask) を使って videodown をインストールできます。Homebrew-Cask を使用するには [Homebrew](https://brew.sh/) がインストールされている必要があります。
...
セットアップウィザード（`videodown-setup-%version%.exe`）をダウンロードして videodown をインストールし、ユーザーごとまたはシステム全体へのインストールを選択します。または、[Chocolatey](https://chocolatey.org/) や [Winget](https://docs.microsoft.com/en-us/windows/package-manager/winget/) などのパッケージマネージャーを使ってインストールすることもできます。
...
Linux、macOS、Windows 用のすべてのバイナリは[リリースページ](https://github.com/cinesim/videodown/releases/latest)からダウンロードできます。お使いのシステム用のバージョンがない場合は、[issue](https://github.com/cinesim/videodown/issues) を開いてください。

## 開発

videodown を自分でビルドしたい場合は、[ビルド手順](../dev/BUILD.md)をご確認ください。

- [ユーザードキュメント](../README.md)
- [開発者ドキュメント](../dev/README.md)

videodown に関してご質問がある場合は、issue を作成していただければ幸いです。その際はデフォルトのフォーマットをご使用ください。もちろん、直接 PR を提出していただけると大変ありがたいです。

## コントリビューション

videodown は開発中です。プルリクエストを作成する前に必ず[コントリビューションガイド](../../.github/CONTRIBUTING.md)をお読みください。videodown に機能を追加したい場合は、[ロードマップ](https://github.com/cinesim/videodown/projects)とオープン issue をご確認ください。

## コントリビューター

videodown にすでに貢献してくださったすべての方々に感謝します[[コントリビューター](https://github.com/cinesim/videodown/graphs/contributors)]。

...
class Muya {
  ...
  setMarkdown(
    ...
    if (blocks && cursor) {
      ...
      // #54 https://github.com/cinesim/videodown/issues/54 This line adds the cursor signature infront that messes up the markdown parsing, so we need to handle the
   ...
   * is currently used to replace a misspelled word in videodown that was selected
   * by Chromium.
   *
...
function getContainer(originContainer, options) {
  ...
  // a custom implementation like in videodown.

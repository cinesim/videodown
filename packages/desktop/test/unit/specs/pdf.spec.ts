...
// `window.videodown.paths` / `window.fileUtils` at call time, all normally
...
vi.hoisted(() => {
  const w = globalThis as unknown as {
    window?: {
      ...
      videodown?: { paths: { userDataPath: string } }
      fileUtils?: {
  ...
  w.window.videodown ??= { paths: { userDataPath: '/userData' } }
...
// never touch `window.fileUtils`/`window.videodown`, unlike a disk theme name)
...
describe('getCssForOptions', () => {
  ...
  it('appends no theme CSS for theme:"default" (disk lookup misses) or {}', async () => {
    ...
    // reads window.videodown.paths + window.fileUtils.isFile (→ false here).

...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to videodown and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'videodown-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: videodown [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`videodown: ${VIDEODOWN_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'videodown-user-data')

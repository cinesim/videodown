...
// legacy videodown inline lexer used to fail before commit 57cd04c5 (Apr 2019,
// videodown PR #957). The new muya's inline lexer (`validateEmphasize` +
...
// the full markdown→HTML pipeline, because the inline lexer is what videodown
...
describe('inline lexer — CommonMark 0.29 spec examples (videodown 57cd04c5)', () => {
...
// Defensive regression for videodown commit d9f64bab (issue #921, PR #947):
...
describe('inline lexer — reference link (videodown d9f64bab)', () => {
...
// Defensive regression for videodown commit 8e32838b (PR #1531) — sup/sub
// inline syntax `^foo^` (superscript) and `~bar~` (subscript). Already
// present in the new muya's inline lexer rules + renderer.
describe('inline lexer — superscript/subscript (videodown 8e32838b)', () => {
...
// Defensive regression for videodown commit c0853f64 (PR #1421):
...
// videodown fix introduced (`top && (pos === 0 || /[* _~(]{1}/...))`).
describe('inline lexer — auto link (videodown c0853f64)', () => {
...
// Defensive regression for videodown commit ad5ddbf9 (GFM example 558, PR #917):
...
describe('inline lexer — GFM link/image title (videodown ad5ddbf9)', () => {
...
// Defensive regression for videodown commit d937fac0 (issue #1071, PR #1072):
...
describe('inline lexer — repeated bold + inline_code (videodown d937fac0 / #1071)', () => {
...
// Defensive regression for videodown commit 57af8304 (issue #1169, PR #1170):
...
// same algorithm.
describe('inline lexer — link / image dest with parens (videodown 57af8304 / #1169)', () => {
    ...
    it('stops at the FIRST matching `)` when more `)` appear later on the line', () => {
        // This is the real shape of the videodown regression: greedy `(.*)` in
...
// Regression for videodown #3778. `lowerPriority` scanned every position for a

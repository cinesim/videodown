...
// Regression for videodown 0baf2e9e / 7de33f11 — "Fix #1390 prevent XSS attack".
//
// videodown's old inline html renderer used the raw tag name as the
...
describe('videodown 0baf2e9e/7de33f11 — inline html tag XSS defenses', () => {
    ...
    describe('data-align attribute whitelist (config side of the fix)', () => {
        it('exposes data-align via ADD_ATTR on EXPORT_DOMPURIFY_CONFIG', () => {
            // videodown added `data-align` to WHITELIST_ATTRIBUTES in the
...
// videodown #3594 / #3697: the editor's live HTML-block preview forbade the

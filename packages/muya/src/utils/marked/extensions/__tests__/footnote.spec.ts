...
// Tests adapted from videodown commit 1ecc3601 (Fix markdown footnote parser, #2840).
// Source: test/unit/specs/markdown-footnotes.spec.js, 510 lines, 23 specs.
//
// The videodown suite emits a flat token stream (`footnote_start` / `footnote_end`).
...
[^foo1]: At vero eos et accusam.`);
    ...
    it('footnote without space between footnote tag and text', () => {
        // videodown fix `\s+` → `\s*` in lexer.js handles this case.
        const tokens = parse(`Lorem [^foo1] ipsum.
...
At vero eos [^foo1]: et accusam.`);
    ...
    // that is closer to CommonMark §4.7 than videodown's old paragraph
...
\\[^1]: foo`);
    ...
    it('backslash-escaped closing bracket is not a footnote (videodown 1ecc3601 fix)', () => {
        ...
        // the rest of the line — exactly the regression videodown fixed.
        const tokens = parse(`foo[^1]
...
`);
    ...
    it('always reports footnote even if referenced identifier differs', () => {
        // Per pandoc spec an unreferenced footnote could be ignored, but videodown
...
// The cases below cover the multi-line footnote-body forms from videodown's

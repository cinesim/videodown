...
// Regression for videodown 8891287b "fix paragraph turn into list bug (#1025)".
//
// In videodown, the old `updateCtrl.updateParagraphToList` ran a loop on
...
// silently lost.
//
// The videodown fix branched: `if (marker) { …split-and-strip… } else
...
describe('replaceBlockByLabel — paragraph→list keeps text verbatim (videodown 8891287b)', () => {
    ...
    it('does not strip a leading `- ` from the bullet-list text (no marker regex)', () => {
        // Pre-fix videodown (front-menu trigger, no marker) dropped this entire
    ...
    it('keeps multi-line text in a single list-item paragraph (no split)', () => {
        // Pre-fix videodown walked `text.split("\n")` and partitioned lines

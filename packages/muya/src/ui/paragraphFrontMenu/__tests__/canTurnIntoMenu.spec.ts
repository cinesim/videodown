...
// Regression for videodown commit 7b7a9424 "should not nest math block into
// other math block (#1153)".
//
// In videodown, `insertContainerBlock(functionType, block)` walked from the
...
// nested inside a math-block. The videodown fix added `getAnchor(block)` +
...
describe('canTurnIntoMenu — no nesting math/code/html/diagram inside themselves (videodown 7b7a9424)', () => {
...
// Regression for videodown commit f00da152 (#812 — "insert table into `table`,
...
// crashed on re-parse. videodown's fix added `getAnchor(block)` + "abort if
// no anchor" gating.
//
...
describe('canTurnIntoMenu — no inserting tables inside non-paragraph containers (videodown f00da152)', () => {

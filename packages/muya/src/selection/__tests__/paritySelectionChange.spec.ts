...
function emitSelectionFor(muya: Muya, content: Content): Record<string, unknown> {
    ...
    if (!payload) throw new Error('selection-change was not emitted');
    return payload;
}

describe('parity PG1: selection-change block affiliation', () => {
    it('PG1: selection-change payload exposes the ancestor block affiliation chain', () => {
    ...
    it('PG1: selection-change exposes the current block markdown type (h1), not just the selection kind', () => {
        const muya = bootMuya('# Heading\n\nbody\n');
        const heading = muya.editor.scrollPage!.firstContentInDescendant()!;
        const payload = emitSelectionFor(muya, heading);
    ...
    it('PG1: selection-change exposes per-endpoint content-leaf block info (type + functionType)', () => {
    ...
    it('PG1: selection-change surfaces list context (ul / li / loose / task) in affiliation', () => {
    ...
    it('PG1: ordered-list items report listItemType "order" (not misclassified as bullet)', () => {
    ...
    it('PG1: loose lists report isLooseListItem true on both the list and the item', () => {

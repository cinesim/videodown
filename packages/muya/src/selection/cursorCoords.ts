...
export function getCursorCoords(preferEnd = false): DOMRect | null {
    ...
    if (sel?.rangeCount) {
        ...
        if (range.getClientRects) {
            ...
            if (rects?.length) rect = preferEnd ? rects[rects.length - 1] : rects[0];
        }
    }
...
export function getCursorYOffset(paragraph: HTMLElement): {
    topOffset: number;
    bottomOffset: number;
} {
    ...
    if (coords == null) return { topOffset: 0, bottomOffset: 0 };

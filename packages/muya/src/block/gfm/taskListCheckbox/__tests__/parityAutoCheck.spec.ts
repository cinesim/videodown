...
function checkboxOf(item: TaskListItem): TaskListCheckbox {
    ...
    (
        item.attachments as unknown as { forEach: (cb: (a: TaskListCheckbox) => void) => void }
    ).forEach((a) => {
        if ((a.constructor as { blockName?: string }).blockName === 'task-list-checkbox') found = a;
    });
    if (!found) throw new Error('task-list-checkbox attachment not found');
    return found;
}
...
function checkedFlags(muya: Muya): boolean[] {
    ...
    const walk = (nodes: ReturnType<Muya['getState']>) => {
        for (const node of nodes) {
            ...
            if (n.name === 'task-list-item') flags.push(!!n.meta?.checked);
            if (Array.isArray(n.children)) walk(n.children as ReturnType<Muya['getState']>);
        }
    };
    walk(muya.getState());
    return flags;
...
describe('parity PG3: autoCheck task-list cascade', () => {
    it('PG3: autoCheck cascades the toggle to descendant task items', async () => {
    ...
    it('PG3: autoCheck re-derives the parent when all descendants become checked', async () => {
        const muya = bootMuya(NESTED_TASKS, { autoCheck: true });
    ...
    it('PG3: autoCheck:false leaves descendants untouched when the parent toggles', async () => {
    ...
    it('PG3: a checked task item serializes as "- [x]"', async () => {
        const muya = bootMuya(NESTED_TASKS, { autoCheck: false });
        const [parent] = taskItems(muya);

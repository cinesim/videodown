...
describe('parity PG6: pasted image FILE routes through imageAction', () => {
    it('PG6: a resolved clipboard image path is persisted via options.imageAction (insert preference)', async () => {
    ...
    it('PG6: the persisted (rewritten) src — not the raw on-disk path — is inserted', async () => {
        const clipboardFilePath = vi.fn().mockResolvedValue('/abs/photo.png');
        const imageAction = vi.fn().mockResolvedValue('assets/photo.png');
        const anchorBlock = makeAnchorBlock('', 0);
        const clipboard = makeClipboard({ clipboardFilePath, imageAction }, anchorBlock);
...
describe('parity PG5: binary/bitmap clipboard image paste', () => {
    it('PG5: a bitmap-only clipboard (no file path) inserts an image via imageAction', async () => {

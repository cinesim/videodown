 ...
 */
function getTableSelectionClipboardData(clipboard: Clipboard): Nullable<IClipboardPayload> {
    const state = clipboard.selection.table.getStateForCopy();
    if (state == null) return null;

    const isSingleCell = state.children.length === 1 && state.children[0].children.length === 1;
    if (isSingleCell) {
...
function resolveSelectionOrder(clipboard: Clipboard, selection: ISelection): Nullable<ICopyOrder> {
    ...
    if (anchorOutMostBlockOffset == null || focusOutMostBlockOffset == null) return null;
...
function truncateLeafState(
    ...
    const text =
        position === 'start' ? leaf.text.substring(offset) : leaf.text.substring(0, offset);
...
function buildPartialContainerState(
    ...
    if (childStates == null) return truncateLeafState(fullState, leaf, offset, position);

    const childBlocks = container.children.map((child) => child);
    const idx = childBlocks.findIndex((child) => child === leaf || leaf.isInBlock(child as Parent));
    if (idx < 0) return fullState;
...
function buildRangeContainerState(
    ...
    if (childStates == null) {
        ...
        if (startLeaf !== endLeaf) return fullState;
    ...
    if (startIdx < 0 || endIdx < 0) return fullState;
    ...
    const startState =
        startChildFull.children != null
            ? buildPartialContainerState(
                  childBlocks[startIdx] as Parent,
                  startLeaf,
                  startOffset,
                  'start',
              )
...
function appendPartialState(
    ...
    if (/block-quote|bullet-list|order-list|task-list/.test(outBlock!.blockName)) {
        copyState.push(buildPartialContainerState(outBlock as Parent, block, offset, position));

        return;
    }

    const truncated =
        position === 'start' ? block.text.substring(offset) : block.text.substring(0, offset);
    ...
    if (
        /setext-heading|code-block|html-block|math-block|frontmatter|diagram/.test(
            outBlock!.blockName,
        ) &&
...
function collectCopyState(order: ICopyOrder): TState[] {
    ...
    if (anchorOutMostBlock === focusOutMostBlock) return collectSameOutMostBlockState(order);
...
export function getClipboardData(clipboard: Clipboard): IClipboardPayload {
    ...
    if (selection == null) return { html: '', text: '' };

    const { isSelectionInSameBlock, anchor, focus } = selection;
    const anchorBlock = anchor.block;
    const focusBlock = focus.block;

    if (anchorBlock == null || focusBlock == null) return { html: '', text: '' };
    ...
    if (order == null) return { html: '', text: '' };
...
export function writeClipboardData(clipboard: Clipboard, event: ClipboardEvent): void {
    ...
    switch (copyType) {
        ...
        case CopyType.COPY_AS_HTML: {
            ...
            event.clipboardData.setData(
                'text/plain',
                getSanitizeClipboardHtml(text, buildHtmlOptions(clipboard.muya.options ?? {})),
            );
            break;
        }

...
<p align="center"><b>Muya</b> — a standalone Markdown editor for the web, extracted from <a href="https://github.com/cinesim/videodown">videodown</a>.</p>
...
This package is developed within the videodown [Bun](https://bun.com/) workspace:
...
Engines: Node ≥18 for consumers and Node ≥20.19 for development, plus Bun ≥1.3. Build target is `chrome70`.
...
```sh
bun run --cwd packages/muya build
```
...
bun run --cwd packages/muya release <version>     # e.g. 0.1.0
```

The release command will prompt for npm 2FA during publishing. If publishing breaks mid-flight after the git tag is already pushed, retry just the upload — the version is already bumped:

```sh
bun publish --cwd packages/muya --tag latest --access public
```
...
**v0.2.0 (in flight)** — the videodown-muya backport batch. 22 PRs (#208–#230) ported the upstream videodown muya tree onto `@muyajs/core` end-to-end:
...
- Parser conformance: CommonMark 0.31 and GFM 0.29-gfm fixture runners (`bun run --cwd packages/muya test:spec`) with a locked baseline (87.7% / 86.3%) and a regression gate via `expected-failures.json`.
...
**What is the relationship between videodown and Muya?**

Muya is derived from videodown. The team's goal is for Muya to live outside the desktop app so it can power web editors as well; the Electron-specific assumptions have been progressively peeled away.

**Does the Muya version track videodown's version?**

No, the two version numbers are independent.

## Built with Muya

- [videodown](https://github.com/cinesim/videodown) — next-generation Markdown editor for macOS, Windows, and Linux.
